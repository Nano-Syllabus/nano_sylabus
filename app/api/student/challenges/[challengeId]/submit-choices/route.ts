...
export async function POST(
  ...
  try {
    ...
    const selections = parseChoiceSelections(body?.answers, questions);
    if (!Object.keys(selections).length) {
      return NextResponse.json({ error: "Choose an answer for at least one question." }, { status: 400 });
    }
    const choices = gradeChallengeChoices(challengeId, questions, selections);
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Could not mark these answers." },
      { status: 502 },
    );
  }
}
