...
// The upstream decides how long these live, and a render still in flight must be
// waited on per request rather than answered from the build's static cache.
export const dynamic = "force-dynamic";

/**
 * `/api/figure/*` — the backend's own figure route, served from this origin.
 *
...
export async function GET(
  ...
  const upstreamPath = assertMediaPath("figure", segments ?? []);
