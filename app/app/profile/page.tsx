    ...
    <>
      ...
      <main className="student-page-frame">
        <header>
          ...
          <h1 className="type-student-page-title mt-2">Your learning profile</h1>
          <p className="type-student-body mt-3 max-w-2xl text-text-secondary">XP grows from completed challenges. Topics turn green as your graded answers reach strong mastery.</p>
        </header>
        <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4" aria-label="Learning totals">
          {cards.map(({ label, value, Icon }) => <article key={label} className="student-surface"><Icon className="size-5 text-text-muted" aria-hidden="true" /><p className="type-student-metric mt-5 tabular-nums">{value}</p><p className="type-student-body mt-1 text-text-secondary">{label}</p></article>)}
        </section>
        <div className="mt-8 grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
          <section className="student-surface" aria-labelledby="mastery-title">
            <div className="flex items-end justify-between gap-4"><div><p className="type-student-eyebrow text-text-muted">Topic progress</p><h2 id="mastery-title" className="type-student-section-title mt-2">Mastery map</h2></div><p className="type-student-metric tabular-nums">{strongPercent}%</p></div>
          ...
          <section className="student-surface" aria-labelledby="xp-title"><h2 id="xp-title" className="type-student-section-title">Recent XP</h2><div className="mt-4 divide-y divide-border">{profile.recentXp.map((event) => <div key={event.id} className="flex items-center gap-3 py-3"><span className="flex size-9 items-center justify-center rounded-full bg-bg-secondary text-xs font-semibold">+{event.points}</span><div className="min-w-0"><p className="text-sm font-medium">{event.reason}</p><p className="type-student-meta mt-0.5 text-text-muted">{new Date(event.createdAt).toLocaleDateString()}</p></div></div>)}{!profile.recentXp.length ? <p className="py-8 text-center text-sm text-text-muted">Complete your first challenge to earn 50 XP.</p> : null}</div></section>
        </div>
      </main>
    </>
  );
