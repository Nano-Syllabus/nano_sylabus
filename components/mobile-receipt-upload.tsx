...
type LinkState =
  ...
  | { kind: "success"; invoiceCode: string; fileName: string };
  ...
  useEffect(() => {
    ...
    async function loadLink() {
      ...
      setState({
        ...
        alreadyUploaded: payload.status === "uploaded",
      });
    }
    void loadLink();
    return () => {
  ...
  async function uploadReceipt(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setState({
        ...
        fileName: payload.fileName || receipt.name,
      });
    } catch (error) {
    ...
    <main className="flex min-h-dvh items-center justify-center bg-bg-secondary px-4 py-8 text-text-primary">
      <section className="w-full max-w-md rounded-3xl border border-border bg-bg-primary p-6 shadow-xl sm:p-8">
        ...
        {state.kind === "loading" ? (
          ...
          >
            <LoaderCircle
              className="size-8 animate-spin text-text-muted motion-reduce:animate-none"
              aria-hidden="true"
            />
          ...
          <div className="flex min-h-72 flex-col items-center justify-center text-center">
            ...
            <p className="mt-4 text-xs text-text-muted">
              Return to your desktop and create a new QR.
            </p>
          </div>
        ) : state.kind === "success" ? (
          <div
          ...
          >
            ...
            <p className="mt-2 max-w-xs text-sm leading-6 text-text-secondary">
              Your desktop will show the receipt automatically. You can close this page and finish
              the payment form there.
            </p>
            <p
              className="mt-4 max-w-full truncate rounded-lg bg-bg-secondary px-3 py-2 text-xs text-text-muted"
            ...
            <button
              ...
              onClick={() => {
                setReceipt(null);
                setSubmitError("");
                setState({ kind: "ready", invoiceCode: state.invoiceCode, alreadyUploaded: true });
              }}
              className="mt-5 inline-flex min-h-11 items-center justify-center rounded-xl border border-border px-4 text-sm font-semibold text-text-primary transition-colors hover:bg-bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-strong focus-visible:ring-offset-2"
            >
          ...
          <form onSubmit={uploadReceipt} className="pt-6">
            ...
            <p className="mt-2 text-sm leading-6 text-text-secondary">
              Take a photo or choose the receipt already saved on this phone.
            </p>

            <label
              htmlFor="phone-payment-receipt"
              className="mt-6 block text-xs font-medium uppercase tracking-wider text-text-secondary"
            >
              Payment receipt *
            </label>
            <Input
            ...
            />
            <p id="phone-receipt-hint" className="mt-2 text-xs text-text-muted">
              JPG, PNG, WebP, or PDF · maximum 5 MB
            </p>
            {state.alreadyUploaded ? (
              <p className="mt-2 text-xs text-amber-700">
                Uploading again will replace the previous phone receipt.
              </p>
            ) : null}
            {submitError ? (
              <p id="phone-receipt-error" role="alert" className="mt-3 text-sm text-destructive">
                {submitError}
              </p>
            ) : null}
