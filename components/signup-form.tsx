...
export function SignupForm({ nextPath }: { nextPath?: string }) {
  ...
  async function onSubmit(event: FormEvent) {
    ...
    if (!marketingConsent) {
      setMarketingConsentError("Please confirm that we can send updates to this number.");
      return;
    }
    ...
    setNotice(
      "Account created. If email confirmation is enabled, confirm your email first and then log in.",
    );
  }
        ...
        <fieldset className="space-y-2">
          ...
          <label className="flex cursor-pointer items-start gap-3 text-sm text-text-secondary">
            <input
              ...
              aria-invalid={marketingConsentError ? "true" : undefined}
              aria-describedby={
                marketingConsentError ? "signup-marketing-consent-error" : undefined
              }
            />
          ...
          {marketingConsentError ? (
            <p
              id="signup-marketing-consent-error"
              role="alert"
