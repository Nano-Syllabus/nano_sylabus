...
describe("unified Figma library", () => {
  ...
  it("uses the real materials and membership APIs and removes Subject Explorer from navigation", () => {
    ...
    expect(settingsForm).not.toContain('label="Faculty"');
    expect(settingsForm).not.toContain('label="Level"');
    expect(settingsForm).not.toContain('label="Last published Board Result"');
    expect(settingsForm).not.toContain('label="Target grade"');
