...
async function deleteRevisionNotesForUsers(
  ...
  const ids = ((result.data || []) as Array<Record<string, unknown>>)
    .filter((row) => {
      ...
      if (courseIdSet.size) {
        return courseId
          ? courseIdSet.has(courseId) && (!keys.size || keys.has(slug))
...
async function deleteStudentPracticeTrails(
    ...
    if (!isMissingTable(sheetsResult.error)) {
      ...
      if (paths.length) {
        const storageResult = await admin.storage
          ...
          .remove(paths);
        // A missing/temporarily unavailable object must not leave the user's
        // database history behind. The row delete below is still authoritative.
        if (storageResult.error) {
          console.warn("[study-trails] answer-sheet storage cleanup failed", storageResult.error);
        }
      }
    }
  ...
  const masteryDelete = await admin
...
async function deleteSubjectChatTrails(
  ...
  const sessionsResult = await admin
    .from("chat_sessions")
    .select("id,user_id,subject_tags,subject_context")
    .in("user_id", userIds);
  ...
  const sessionIds = ((sessionsResult.data || []) as Array<Record<string, unknown>>)
    .filter((row) => sessionBelongsToSubjects(row, keys))
    .map((row) => String(row.id || ""))
    .filter(Boolean);
  ...
  const classroomIds = ((classroomsResult.data || []) as Array<Record<string, unknown>>)
    .filter((row) => {
      ...
      return courseIdSet.size
        ? classroomCourseId
          ? courseIdSet.has(classroomCourseId)
...
export async function clearStudentStudyTrails(
  ...
  if (!users.length || !cleanSubjects.length) return;

  await deleteRevisionNotesForUsers(admin, users, cleanSubjects, unique(courseIds));
  await deleteSubjectChatTrails(admin, users, cleanSubjects);
  await deleteStudentPracticeTrails(admin, users, cleanSubjects);
...
export async function clearTeacherCourseTrails(
  ...
  const users = unique([...enrolledStudentIds, teacherUserId || ""]);
  await clearStudentStudyTrails(admin, users, subjects, [courseId], teacherId);
  await deleteTeacherExamTrails(
    admin,
    teacherId,
    unique(subjects.map((subject) => subject.subjectSlug)),
